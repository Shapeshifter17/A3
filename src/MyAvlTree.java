public class MyAvlTree {
    ...
    {
        rootNode = insertElement(key, value, rootNode);
    }
    ...
    private Node insertElement(String key, String value, Node node)
    {
        ...
        {
            node.leftChild = insertElement( key, value, node.leftChild );
            if( getHeight( node.leftChild ) - getHeight( node.rightChild ) == 2 )
                if( Integer.parseInt(key) < Integer.parseInt(node.leftChild.key))
        ...
        {
            node.rightChild = insertElement( key, value, node.rightChild );
    ...
    {
        return searchElement(rootNode, key);
    }

    private boolean searchElement(Node head, String key)
    {
        boolean check = false;
        while ((head != null) && !check)
        {
            int headElement = Integer.parseInt(head.key);
            if (Integer.parseInt(key) < headElement)
                head = head.leftChild;
            else if (Integer.parseInt(key) > headElement)
